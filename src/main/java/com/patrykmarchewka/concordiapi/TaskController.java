...
public class TaskController {
    ...
    public ResponseEntity<APIResponse<Set<?>>> getAllTasks(@PathVariable long teamID,Authentication authentication){
        ControllerContext context = ControllerContext.forTasks(authentication,teamID,teamService,teamUserRoleService);
        if (!context.getUserRole().isAllowedBasic()){
            throw new NoPrivilegesException();
        }

        return ResponseEntity.ok(new APIResponse<>("All tasks available", taskService.getAllTasksRoleController(context.getUserRole(),taskService, context.getTeam(), context.getUser())));
    }
    ...
    public ResponseEntity<APIResponse<Set<?>>> getAllTasksAssignedToMe(@PathVariable long teamID,Authentication authentication){
        ...
        return ResponseEntity.ok(new APIResponse<>("Tasks assigned to me",taskService.getMyTasksRoleController(context.getUserRole(), context.getUser())));
    ...
    public ResponseEntity<APIResponse<Object>> getTaskByID(@PathVariable long teamID,@PathVariable long ID, Authentication authentication){
        ControllerContext context = ControllerContext.forSubtasks(authentication,teamID,ID,teamService,taskService,teamUserRoleService);
        return ResponseEntity.ok(new APIResponse<>("Task details", taskService.getInformationAboutTaskRoleController(context.getUserRole(), context.getTask(), context.getUser())));
    ...
    public ResponseEntity<APIResponse<TaskMemberDTO>> putTask(@PathVariable long teamID, @PathVariable long ID, @RequestBody @Validated(OnCreate.class) TaskRequestBody body, Authentication authentication){
        ControllerContext context = ControllerContext.forSubtasks(authentication,teamID,ID,teamService,taskService,teamUserRoleService);
        if (!taskService.putTaskRoleController(context.getUserRole(), context.getTask(), context.getUser())){
            throw new NoPrivilegesException();
        }
    ...
    public ResponseEntity<APIResponse<TaskMemberDTO>> patchTask(@PathVariable long teamID,@PathVariable long ID, @RequestBody TaskRequestBody body, Authentication authentication){
        ...
        return ResponseEntity.ok(new APIResponse<>("Task updated",new TaskMemberDTO(taskService.partialUpdate(context.getTask(), body, context.getTeam()))));
    }
    ...
    public ResponseEntity<?> addOneUserToTask(@PathVariable long teamID, @PathVariable long ID,@PathVariable long userID, Authentication authentication){
        ControllerContext context = ControllerContext.forTasksWithUser(authentication,teamID,teamService,teamUserRoleService, userService.getUserByID(userID));

        if (!teamUserRoleService.checkRoles.test(context.getUserRole(),context.getOtherRole())){
            throw new NoPrivilegesException();
        }

        taskService.addUserToTask(context.getTeam(), ID, userService.getUserByID(userID));
    ...
    public ResponseEntity<?> deleteOneUserFromTask(@PathVariable long teamID, @PathVariable long ID,@PathVariable long userID, Authentication authentication){
        ...
        taskService.removeUserFromTask(context.getTeam(), ID, userService.getUserByID(userID));
